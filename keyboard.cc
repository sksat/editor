...
//	for(int i=0;i<7;i++) printf("%x", static_cast<unsigned char>(buf[i]));
	...
	if(c1 == 0x00){
		...
		if(c2 == 0x31){
			...
			k.extend = true;
			if(c2 == 0x35) k.c = ext::PgUp; // PgUp
			else k.c = ext::PgDown; // PgDown
			...
			case 0x44: // Left
				k.c = ext::Up + (c2-0x41);
				break;
			case 0x46: k.c = ext::End; break; // End
			case 0x48: k.c = ext::Home; break; // Home
			default:
				k.c = 0x00; // unknwon
				break;
			}
...
std::string key2str(const Key &k){
	...
	else if(k.is_print) str+= k.c;
	else if(k.extend){
		switch(k.c){
		#define EXT_KEY(key) case (ext::key): str+= (#key); break;
		EXT_KEY(Up);
		EXT_KEY(Down);
		EXT_KEY(Right);
		EXT_KEY(Left);
